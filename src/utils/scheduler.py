...
def cosine_lr(optimizer, base_lr, warmup_length, steps, min_lr=0.002):
    def _lr_adjuster(step):
        if step < warmup_length:
            lr = _warmup_lr(base_lr, warmup_length, step)
        else:
            e = step - warmup_length  # current step
            es = steps - warmup_length  # total steps
            # lr = 0.5 * (1 + np.cos(np.pi * e / es)) * base_lr
            lr = min_lr + 0.5 * (base_lr - min_lr) * (1 + np.cos(np.pi * e / es))
        assign_learning_rate(optimizer, lr)
        return lr
