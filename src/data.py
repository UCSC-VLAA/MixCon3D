...
class Four(Dataset):
    ...
    def get_objaverse(self, meta):
        uid = meta["id"]
        # meta['data_path'] == '/mnt/data/objaverse-processed/merged_for_training_final/Objaverse/000-041/xxx.npy
        # data = np.load(meta['data_path'], allow_pickle=True).item()
        path = meta['data_path'].replace("mnt", self.folder)
        ...
        # If you want to create multi-view images, the below sample mechanisms ought to be modified
        if self.config.dataset.image_feat_mode == "Original":
            if np.random.rand() < 0.5:
                img_feat = data['thumbnail_feat']
            else:
                idx = np.random.randint(data['image_feat'].shape[0])
                img_feat = data["image_feat"][idx]
            # Return [D] tensor TODO-Done: I think returning [1, D] may be more compatible with the modification
            # img_feat = torch.from_numpy(img_feat).type(torch.float32).reshape(-1).unsqueeze(0)
            # Return [1, D] tensor
            img_feat = torch.from_numpy(img_feat).type(torch.float32).reshape(-1).unsqueeze(0)
        elif self.config.dataset.image_feat_mode == "Multiple":
            image_amount = self.config.dataset.image_amount
            if np.random.rand() < 0.5:
                img_thu_feat = data['thumbnail_feat'].reshape(1, -1)
                idx = np.random.randint(0, data['image_feat'].shape[0], size=int(image_amount-1))
                img_prj_feat = data["image_feat"][idx]
                img_feat = np.concatenate((img_thu_feat, img_prj_feat), axis=0)
            else:
                idx = np.random.randint(0, data['image_feat'].shape[0], size=int(image_amount))
                img_feat = data["image_feat"][idx]
            # Return [1, image_amount, D] tensor
            img_feat = torch.from_numpy(img_feat).type(torch.float32).unsqueeze(0)
            #print("The shape of img_feature in Multiple mode is ", img_feat.shape)
    ...
    def get_others(self, meta):
        ...
        if self.config.dataset.image_feat_mode == "Original":
            idx = np.random.randint(data['image_feat'].shape[0])
            img_feat = data["image_feat"][idx]
            # Return [1, D] tensor TODO: I think returning [1, D] may be more compatible with the modification
            img_feat = torch.from_numpy(img_feat).type(torch.float32).reshape(-1).unsqueeze(0)
        elif self.config.dataset.image_feat_mode == "Multiple":
            image_amount = self.config.dataset.image_amount
            idx = np.random.randint(0, data['image_feat'].shape[0], size=int(image_amount))
            img_feat = data["image_feat"][idx]
            # Return [image_amount, D] tensor
            img_feat = torch.from_numpy(img_feat).type(torch.float32).unsqueeze(0)
            # print("The shape of img_feature in Multiple mode is ", img_feat.shape)
...
def make(config, phase, rank, world_size):
    if config.dataset.name == "Four":
        ...
        data_loader = DataLoader(
            ...
            drop_last=True,
            sampler=sampler,
            # Yipeng: Add this line to analyze the "memory increasing" problem
            # The persistent_workers=True works for alleviating the "memory increasing" problem
...
class ObjaverseLVIS(Dataset):
    ...
    def __getitem__(self, index: int):
        # path = meta['data_path'].replace("mnt", "data3")
        # data = np.load(path, allow_pickle=True).item()
        # data = np.load(self.split[index]['data_path'], allow_pickle=True).item()
        path = self.split[index]['data_path'].replace("mnt", self.folder)
        data = np.load(path, allow_pickle=True).item()
        n = data['xyz'].shape[0]
        # if n != self.num_points:
        idx = random.sample(range(n), self.num_points)
        ...
        # Now the img_feat can be obtained directly from the meta_data
        # So for Objaverse, we can use the img_feat for inference
        #if self.config.training.debug:
        #    print("testing the key img_feat in meta_data")
        # If you want to create multi-view images, the below sample mechanisms ought to be modified
        if self.config.dataset.inference_image_feat_mode == "Original":
            idx = np.random.randint(data['image_feat'].shape[0])
            img_feat = data["image_feat"][idx]
            # Return [1, D] tensor TODO: I think returning [1, D] may be more compatible with the modification
            img_feat = torch.from_numpy(img_feat).type(torch.float32).reshape(-1).unsqueeze(0)
        elif self.config.dataset.inference_image_feat_mode == "Multiple":
            image_amount = self.config.dataset.image_amount
            idx = np.random.randint(0, data['image_feat'].shape[0], size=int(image_amount))
            img_feat = data["image_feat"][idx]
            # Return [image_amount, D] tensor
